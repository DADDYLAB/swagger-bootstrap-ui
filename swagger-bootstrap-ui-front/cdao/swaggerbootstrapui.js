...
(function ($) {
    ...
    SwaggerBootstrapUi.prototype.createSettingsPage = function () {
        ...
        setTimeout(function () {
            if (!that.tabExists(tabId)) {
                that.log("settings-----------------")
                that.log(that.settings)
                var nsettings = $.extend({}, that.settings, {
                    i18n: that.i18n.instance
                })
    ...
    SwaggerBootstrapUi.prototype.createApiInfoTable = function (apiInfo, menu) {
        ...
        that.storeCacheOpenApiTableApis({
            tabId: tabId
        })
        var i18n = that.i18n.instance;
    ...
    SwaggerBootstrapUi.prototype.createDebugTab = function (apiInfo, menu) {
        ...
        apiInfo.i18n = i18n;
        var host = (that.currentInstance.host || '').split(',');
        apiInfo.host = host;
        ...
        btnRequest.on("click", function (e) {
            ...
            paramBody.find("tr").each(function () {
                ...
                if (cked) {
                    //如果选中
                    var trdata = {
                        name: paramtr.find("td:eq(2)").find("input").val(),
                        in: paramtr.data("in"),
                        ...
                        if (required) {
                            if (!reqflag) {
                                ...
                                if (value == null || value == "") {
                                    ...
                                    validateobj = {
                                        message: des + i18n.message.debug.fieldNotEmpty
                                    };
                                    return false;
                                }
                            }
     ...
     * @param formRequest
     */
    SwaggerBootstrapUi.prototype.createResponseElement = function (index, apiInfo, headerparams, reqdata, paramBodyType, url, fileUploadFlat, formCurlParams, xhr, data, startTime, allheaders, formRequest, binaryType) {
        var that = this;
        var i18n = that.i18n.instance;
        var apiKeyId = apiInfo.id;
    ...
    SwaggerBootstrapUi.prototype.tabCloseEventsInit = function () {
        ...
        $(document).delegate(".icon-sbu-tab-close", "click", function () {
            ...
            if (tabId) {
                that.deleteCacheOpenApiTableApis({
                    tabId: tabId
                });
            }
        })
    ...
    SwaggerBootstrapUi.prototype.resetAuthEvent = function (tabContentId) {
        ...
        that.getDoc().find("#" + tabContentId).find(".btn-reset-auth").on("click", function (e) {
            e.preventDefault();
            layer.confirm(i18n.message.auth.confirm, {
                title: i18n.message.layer.title,
                btn: [i18n.message.layer.yes, i18n.message.layer.no]
            }, function (index) {
                $.each(that.currentInstance.securityArrs, function (i, sa) {
                    sa.value = "";
                    //that.updateGlobalParams(sa,"securityArrs");
    ...
    SwaggerBootstrapUi.prototype.createGroupElement = function () {
        ...
        var islsSelectedName = JSON.parse(localStorage.getItem('selectedName'));
        // console.log(islsSelectedName);
        $.each(that.instances, function (i, group) {
            ...
            if (islsSelectedName == group.name) {
                var groupOption = $("<option data-url='" + url + "' data-name='" + group.name + "'selected >" + group.name + "</option>");
            } else {
        ...
        groupSele.on("change", function () {
            localStorage.removeItem('instance', 'selectedName');
            var t = $(this);
        ...
        //默认加载第一个url
        var isLocalStorage = JSON.parse(localStorage.getItem('instance'));
        // console.log(isLocalStorage);
        if (
            isLocalStorage
        ) {
            that.analysisApi(isLocalStorage);
        } else {
